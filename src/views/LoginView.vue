...
watch(username, () => { usernameError.value = false })
watch(password, () => { passwordError.value = false })
watch(authCode, () => { authCodeError.value = false })
      ...
      <div class="background-container container-fluid">
        <div class="login-container">
          ...
          <form @submit.prevent="login" class="login-form">
            <h2 class="text-center fw-bold fs-6 mb-3">Log in with your TRAPAgram account</h2>
            <input v-model="username" type="text" class="container-fluid mb-2" :class="{ 'error': usernameError }" placeholder="Username, phone or email" autocomplete="off">
            <input v-model="password" type="password" class="container-fluid mb-2" :class="{ 'error': passwordError }" placeholder="Password" autocomplete="off">
            <input v-model="authCode" v-if="enable2fa" class="container-fluid mb-2" :class="{ 'error': authCodeError }" placeholder="Authentication Code" autocomplete="off" type="text">
            <button type="submit" class="login-btn container-fluid">Log in</button>
            <RouterLink :to="{ name: 'register' }" class="d-block text-decoration-none container-fluid text-center forgot-link">Don't have an account?</RouterLink>
          </form>
